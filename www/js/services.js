...
.service('OfferService', function ($http, $q, API_URL, IS_DUMMY){
    
	this.getOffers = function(offerCategory) {
		var deferred = $q.defer();
        
        if(IS_DUMMY){            
             return getPromise(deferred, dummyData.offers);
        }       
        $http.get(API_URL+ (offerCategory==='latest'?'/ltestOffers':'/bestOffers'))
    ...
    this.getOfferById=function(id) {
        ...
        if(IS_DUMMY){            
             return getPromise(deferred, dummyData.offerByid);
        }       
        $http.get(API_URL+ '/getOfferById/'+id)
        .success(function(data) {
            deferred.resolve(data);
        })
        .error(function(data) {
...
.service('StoresService', function ($http, $q, API_URL, IS_DUMMY){
     
     this.getStores=function() {
        var deferred = $q.defer();
        
        if(IS_DUMMY){            
             return getPromise(deferred, dummyData.stores);
        }       
     ...
     this.getStore=function(id) {
        ...
        if(IS_DUMMY){            
             return getPromise(deferred, dummyData.stores[0]);
        }       
        $http.get(API_URL+ '/stores/'+id)
        .success(function(data) {
            deferred.resolve(data);
        })
        .error(function(data) {
...
.service('CategoryService', function name($http, $q, API_URL, IS_DUMMY) {
        this.getCategories=function() {
            var deferred = $q.defer();
            
            if(IS_DUMMY){            
                return getPromise(deferred, dummyData.categories);
            }       
            $http.get(API_URL+ '/getCategories/')
            .success(function(data) {
                deferred.resolve(data);
            })
            .error(function(data) {
